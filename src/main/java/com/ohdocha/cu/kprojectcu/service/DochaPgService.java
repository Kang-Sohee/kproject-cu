...
import com.carssum.csdeal.carSearch.CarssumCarSearchPaymentDetailDto;
import com.carssum.csdeal.carSearch.CarssumCarSearchService;
import com.carssum.csdeal.estimate.CarssumQuoteUserDto;
import com.carssum.csdeal.estimate.CarssumQuoteUserService;
import com.carssum.csdeal.rentcar.CarssumRentcarService;
import com.carssum.csdeal.userInfo.CarssumUserInfoDto;
import com.carssum.csdeal.userInfo.CarssumUserInfoService;
import com.carssum.csdeal.util.CarssumMap;
import com.carssum.csdeal.util.KeyMaker;
...
public class CarssumPgService {
	...
	CarssumPaymentService paymentService;

	@Resource(name="QuoteUser")
	CarssumQuoteUserService quoteUserservice;

	@Resource(name="userInfo")
	CarssumUserInfoService userinfoService;

	@Resource(name="carssumRentcarService")
	CarssumRentcarService rentcarService;
	
	
	@Resource(name="carSearch")
	CarssumCarSearchService carSearchService;

	/**
	 * 
	 * @param payment
	 * @return CarssumMap <br/>
	 ...
	 */
	@Transactional
	public CarssumMap payres(CarssumPaymentLgdDto payment, Authentication authentication) throws Exception {

		CarssumMap param = new CarssumMap();
		...
		if (xpay.TX()) {
			...
			if ("0000".equals(xpay.m_szResCode)) {
				...
				try {
					...
					CarssumPaymentLogDto paymentLogDto = new CarssumPaymentLogDto();
					...
					//selectUserQuoteInfo
					CarssumMap paramMap = new CarssumMap();
					...
					CarssumPaymentDto carssumPaymentReserveMasterDto = new CarssumPaymentDto();
					CarssumUserInfoDto paramUserInfoDto = new CarssumUserInfoDto();
					CarssumUserInfoDto responseUserInfoDto = new CarssumUserInfoDto();
					
					CarssumPaymentReserveDto quoteUserDto = new CarssumPaymentReserveDto();
					...
					if(StringUtil.isEmpty(payment.getCarSearchYn()) || "N".equals(payment.getCarSearchYn())) {
						...
						CarssumMap userParam = new CarssumMap();
						...
						*/
						...
						carssumPaymentReserveMasterDto.setRmIdx(rmIdx);
						carssumPaymentReserveMasterDto.setReserveTypeCode(reserveTypeCode);
						carssumPaymentReserveMasterDto.setReserveStatusCode(reserveStatusCode); 
						carssumPaymentReserveMasterDto.setLongTermYn(longTerm);
						carssumPaymentReserveMasterDto.setUrIdx(payment.getUrIdx());
						carssumPaymentReserveMasterDto.setReserveUserName(reserveUserName);
						carssumPaymentReserveMasterDto.setRentStartDay(rentStartDay);
						carssumPaymentReserveMasterDto.setRentEndDay(rentEndDay);
						carssumPaymentReserveMasterDto.setRentStartTime(rentStartTime);
						carssumPaymentReserveMasterDto.setRentEndTime(rentEndTime);
						carssumPaymentReserveMasterDto.setDeliveryTypeCode(deliveryTypeCode);
						carssumPaymentReserveMasterDto.setDeliveryAddr(deliveryAddr);
						carssumPaymentReserveMasterDto.setReturnTypeCode(returnTypeCode);
						carssumPaymentReserveMasterDto.setReturnAddr(returnAddr);
						carssumPaymentReserveMasterDto.setCrIdx(crIdx);
						carssumPaymentReserveMasterDto.setCarTypeCode(carTypeCode);
						carssumPaymentReserveMasterDto.setRtIdx(rtIdx);
						carssumPaymentReserveMasterDto.setCompanyName(companyName);
						carssumPaymentReserveMasterDto.setReserveDate(reserveDate);
						carssumPaymentReserveMasterDto.setPaymentDate(paymentDate);
						carssumPaymentReserveMasterDto.setCarDeposit(carDeposit);
						carssumPaymentReserveMasterDto.setRentFee(rentFee);
						carssumPaymentReserveMasterDto.setInsuranceFee(insuranceFee);
						carssumPaymentReserveMasterDto.setDiscountFee(discountFee);
						carssumPaymentReserveMasterDto.setPaymentAmount(paymentAmount);
						carssumPaymentReserveMasterDto.setCancelFee(cancelFee);
						carssumPaymentReserveMasterDto.setCancelAmount(cancelAmount);
						carssumPaymentReserveMasterDto.setCancelReason(cancelReason);
						carssumPaymentReserveMasterDto.setQuIdx(quIdx);
						carssumPaymentReserveMasterDto.setFirstDriverName(firstDriverName);
						carssumPaymentReserveMasterDto.setUlIdx1(ulIdx1);
						carssumPaymentReserveMasterDto.setSecondDriverName(secondDriverName);
						carssumPaymentReserveMasterDto.setUlIdx2(ulIdx2);
						carssumPaymentReserveMasterDto.setReserveMEtc(reserveMEtc);
						carssumPaymentReserveMasterDto.setRegId(urIdx);
						carssumPaymentReserveMasterDto.setRegDt(regDt);
						carssumPaymentReserveMasterDto.setModId(modId);
						carssumPaymentReserveMasterDto.setModDt(modDt);
						carssumPaymentReserveMasterDto.setDelYn(delYn);
						...
						CarssumUserInfoDto loginSessionInfo = (CarssumUserInfoDto) authentication.getPrincipal();
						...
						CarssumMap reqParam = new CarssumMap();
						reqParam.set("crIdx", crIdx);
						reqParam.set("rentStartDate", rentStartDt);
						reqParam.set("rentEndDate", rentEndDt);
						List<CarssumCarSearchPaymentDetailDto> carInfoList =  carSearchService.selectCarSearchDetail(reqParam);
						//배차방법이 지점방문이면 지점주소를 넣음
						if("OF".equals(deliveryTypeCode)) {
						...
						carssumPaymentReserveMasterDto.setRmIdx(rmIdx);
						carssumPaymentReserveMasterDto.setUrIdx(urIdx);
						carssumPaymentReserveMasterDto.setReserveTypeCode(reserveTypeCode);
						carssumPaymentReserveMasterDto.setReserveStatusCode(reserveStatusCode); 
						carssumPaymentReserveMasterDto.setLongTermYn(longTerm);
						carssumPaymentReserveMasterDto.setUrIdx(payment.getUrIdx());
					...
					CarssumPaymentReserveDto carssumPaymentReserveDto = new CarssumPaymentReserveDto();

					//rmIdx = KeyMaker.getInsetance().getKeyDeafult("RM");
					reIdx = xpay.Response("LGD_OID", 0); //KeyMaker.getInsetance().getKeyDeafult("RE"); 

					reserveStatusCode = "QC";
					carssumPaymentReserveDto.setReIdx(reIdx);
					carssumPaymentReserveDto.setRmIdx(rmIdx);
					carssumPaymentReserveDto.setReserveStatusCode(reserveStatusCode);
					carssumPaymentReserveDto.setRentStartDay(rentStartDay);
					carssumPaymentReserveDto.setRentEndDay(rentEndDay);
					carssumPaymentReserveDto.setRentStartTime(rentStartTime);
					carssumPaymentReserveDto.setRentEndTime(rentEndTime);
					carssumPaymentReserveDto.setCarDeposit(carDeposit);
					carssumPaymentReserveDto.setRentFee(rentFee);
					carssumPaymentReserveDto.setInsuranceFee(insuranceFee);
					carssumPaymentReserveDto.setDiscountFee(discountFee);
					carssumPaymentReserveDto.setPaymentAmount(paymentAmount);
					carssumPaymentReserveDto.setCancelFee(cancelFee);
					carssumPaymentReserveDto.setCancelAmount(cancelAmount);
					carssumPaymentReserveDto.setReserveEtc("");
					carssumPaymentReserveDto.setRegId(urIdx);
					carssumPaymentReserveDto.setRegDt(regDt);

					//4. insertPaymentDetail
					ArrayList<CarssumPaymentDetailDto> payDetailList = new ArrayList<CarssumPaymentDetailDto>();
					...
					//보험료 존재여부 확인
					if(!StringUtil.isEmpty(carssumPaymentReserveMasterDto.getInsuranceFee())) {
						//보혐료가 0보다 클경우 PAYDETAIL에 보험료 항목 등록
						if(StringUtil.isEmptyConvertInt(carssumPaymentReserveMasterDto.getInsuranceFee(), 0) > 0) {

							intInsuranceFee = StringUtil.isEmptyConvertInt(payment.getInsuranceFee(), 0);

							CarssumPaymentDetailDto carssumPaymentDetailDto = new CarssumPaymentDetailDto(); 

							String pdIdx 			= KeyMaker.getInsetance().getKeyAddRandomDigitForLoof("PD", 3);

							carssumPaymentDetailDto.setPdIdx(pdIdx);
							carssumPaymentDetailDto.setRmIdx(rmIdx);
							carssumPaymentDetailDto.setUrIdx(payment.getUrIdx());
							carssumPaymentDetailDto.setPaymentAmount(payment.getInsuranceFee());
							//PG사에 따른 코드
							carssumPaymentDetailDto.setPgCode("LG");
							//결제수단 카드이므로 CD, 차후 결제수단 추가시 로직 수정 필요
							carssumPaymentDetailDto.setPaymentTypeCode("CD");
							//결제금액종류 보험금액 코드 등록
							carssumPaymentDetailDto.setPaymentKindCode("IS");
							carssumPaymentDetailDto.setApprovalNumber(approvalNumber);
							carssumPaymentDetailDto.setPaymentDate(paymentDate);

							payDetailList.add(carssumPaymentDetailDto);
						}

					}

					//보증금 존재여부 확인
					if(!StringUtil.isEmpty(carssumPaymentReserveMasterDto.getCarDeposit())) {
						//보증금 0보다 클경우 PAYDETAIL에 보증금 항목 등록
						if(StringUtil.isEmptyConvertInt(carssumPaymentReserveMasterDto.getCarDeposit(), 0) > 0) {
							...
							carssumPaymentDetailDto.setRmIdx(rmIdx);
							carssumPaymentDetailDto.setUrIdx(payment.getUrIdx());
							carssumPaymentDetailDto.setPaymentAmount(payment.getCarDeposit());
							//PG사에 따른 코드
							carssumPaymentDetailDto.setPgCode("LG");
							//결제수단 카드이므로 CD, 차후 결제수단 추가시 로직 수정 필요
							carssumPaymentDetailDto.setPaymentTypeCode("CD");
							//결제금액종류 보증금 코드 등록
							carssumPaymentDetailDto.setPaymentKindCode("DP");
							carssumPaymentDetailDto.setApprovalNumber(approvalNumber);
							carssumPaymentDetailDto.setPaymentDate(paymentDate);
					...
					//대여료 존재여부 확인
					if(!StringUtil.isEmpty(carssumPaymentReserveMasterDto.getRentFee())) {
						//대여료가 0보다 클경우 PAYDETAIL에 대여료 항목 등록
						if(StringUtil.isEmptyConvertInt(carssumPaymentReserveMasterDto.getRentFee(), 0) > 0) {
							CarssumPaymentDetailDto carssumPaymentDetailDto = new CarssumPaymentDetailDto();
							...
							carssumPaymentDetailDto.setRmIdx(rmIdx);
							carssumPaymentDetailDto.setUrIdx(payment.getUrIdx());
							carssumPaymentDetailDto.setPaymentAmount(Integer.toString(intPaymentAmount));
							//PG사에 따른 코드
							carssumPaymentDetailDto.setPgCode("LG");
							//결제수단 카드이므로 CD, 차후 결제수단 추가시 로직 수정 필요
							carssumPaymentDetailDto.setPaymentTypeCode("CD");
							//결제금액종류 대여금액 코드 등록
							carssumPaymentDetailDto.setPaymentKindCode("RT");
							carssumPaymentDetailDto.setApprovalNumber(approvalNumber);
							carssumPaymentDetailDto.setPaymentDate(paymentDate);
					...
					//취소수수료 존재여부 확인
					if(!StringUtil.isEmpty(carssumPaymentReserveMasterDto.getCancelFee())) {
						...
						if(StringUtil.isEmptyConvertInt(carssumPaymentReserveMasterDto.getInsuranceFee(), 0) > 0) {
							...
							carssumPaymentDetailDto.setRmIdx(rmIdx);
							carssumPaymentDetailDto.setUrIdx(payment.getUrIdx());
							carssumPaymentDetailDto.setPaymentAmount(payment.getCancelFee());
							//PG사에 따른 코드
							carssumPaymentDetailDto.setPgCode("LG");
							//결제수단 카드이므로 CD, 차후 결제수단 추가시 로직 수정 필요
							carssumPaymentDetailDto.setPaymentTypeCode("CD");
							//결제금액종류 취소수수료 코드 등록
							carssumPaymentDetailDto.setPaymentKindCode("CF");
							carssumPaymentDetailDto.setApprovalNumber(approvalNumber);
							carssumPaymentDetailDto.setPaymentDate(paymentDate);
					...
					//취소금액 존재여부 확인
					if(!StringUtil.isEmpty(carssumPaymentReserveMasterDto.getCancelAmount())) {
						...
						if(StringUtil.isEmptyConvertInt(carssumPaymentReserveMasterDto.getInsuranceFee(), 0) > 0) {
							...
							carssumPaymentDetailDto.setRmIdx(rmIdx);
							carssumPaymentDetailDto.setUrIdx(payment.getUrIdx());
							carssumPaymentDetailDto.setPaymentAmount(payment.getCancelAmount());
							//PG사에 따른 코드
							carssumPaymentDetailDto.setPgCode("LG");
							//결제수단 카드이므로 CD, 차후 결제수단 추가시 로직 수정 필요
							carssumPaymentDetailDto.setPaymentTypeCode("CD");
							//결제금액종류 취소금액 코드 등록
							carssumPaymentDetailDto.setPaymentKindCode("CN");
							carssumPaymentDetailDto.setApprovalNumber(approvalNumber);
							carssumPaymentDetailDto.setPaymentDate(paymentDate);
					...
					//5.  CDT_RESERVE_MASTER의 PDIDX를  CarssumPaymentDetailDto로부터 UPDATE 한다.

					CarssumMap insertPaymentRes = insertPaymentData(carssumPaymentReserveMasterDto, carssumPaymentReserveDto, payDetailList, qrIdx, payment);
					...
					//System.out.println("CarssumPaymentLogDto update" + carssumPaymentDetailDto.getPlIdx());

					//저장 성공 후 log Table pdidx update
					//CarssumPaymentLogDto LogParamDto = new CarssumPaymentLogDto();
					//LogParamDto.setPlIdx(PL_IDX);
					//LogParamDto.setPdIdx(carssumPaymentDetailDto.getPdIdx());

					System.out.println("CarssumPaymentLogDto update : " + PL_IDX);
					...
					param.set("urIdx", carssumPaymentReserveDto.getRmIdx());

					payment.setRmIdx(carssumPaymentReserveDto.getRmIdx());
					...
					if (!isDBOK) {							
						...
						int errCd = saveErrorLog(xpay, new CarssumPaymentLogDto());
	...
	public void insertLog(XPayClient xpay, String division) {
		...
		CarssumMap resData = new CarssumMap();
	...
	public CarssumMap insertPaymentData(CarssumPaymentDto carssumPaymentReserveMasterDto, CarssumPaymentReserveDto carssumPaymentReserveDto,
			ArrayList<CarssumPaymentDetailDto> carssumPaymentDetailDtoList, String qrIdx, CarssumPaymentLgdDto lgDto) {
		CarssumMap resData = new CarssumMap();
		...
		try {
			...
			//마스터 생성
			paymentService.insertReserveMaster(carssumPaymentReserveMasterDto);
		
			//예약 로그 생성
			paymentService.insertReserve(carssumPaymentReserveDto);
		}catch(Exception e){
		...
		//payDetail 생성
		try {
			for(CarssumPaymentDetailDto dto : carssumPaymentDetailDtoList) {
		...
		try {
			...
			if(!"Y".equals(lgDto.getCarSearchYn())) {
				...
				CarssumMap paramQuoteUser = new CarssumMap();
				paramQuoteUser.set("quoteStatus", "QC");
				paramQuoteUser.set("urIdx", carssumPaymentReserveMasterDto.getUrIdx());
				paramQuoteUser.set("quIdx", carssumPaymentReserveMasterDto.getQuIdx());
				...
				CarssumMap paramRentCar = new CarssumMap();
				paramRentCar.set("quoteStatus", "QC");
				paramRentCar.set("urIdx", carssumPaymentReserveMasterDto.getUrIdx());
				paramRentCar.set("quIdx", carssumPaymentReserveMasterDto.getQuIdx());
				paramRentCar.set("qrIdx", qrIdx);
				paymentService.updateCompliteQuoteRentCompany(paramRentCar);
	
				//선택된 회원사 견적 제외 나머지 QB로 상태 업데이트
			
				CarssumMap paramNotChoiceRentCar = new CarssumMap();
	
				paramNotChoiceRentCar.set("quoteStatus", "QB");
				paramNotChoiceRentCar.set("quIdx", carssumPaymentReserveMasterDto.getQuIdx());
	...
	private int saveErrorLog(XPayClient xpay, CarssumPaymentLogDto paymentLogDto) {
