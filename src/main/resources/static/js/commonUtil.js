...
function cf_Calculate_between_date_period_Momth(startTime, EndTime) {
	 ...
	 var startDate = new Date(parseInt(startTime.substring(0,4), 10),
	  ...
	  var endDate   = new Date(parseInt(endTime.substring(0,4), 10),
	             	 ...
	             	 parseInt(endTime.substring(12,14), 10));f
}
...
function getCodeValue(strCode, cmd){
	...
	if( cmd == 'FL' ){
		switch(strCode){
			case 'FGS' : returnValue = '휘발류'; break;
			case 'FDS' : returnValue = '경유'; break;
			case 'FLP' : returnValue = 'LPG'; break;
			case 'FHB' : returnValue = '하이브리드'; break;
			case 'FEL' : returnValue = '전기'; break;
			case 'FHD' : returnValue = '수소'; break;
			default : break;
		}
	}
	return returnValue;
